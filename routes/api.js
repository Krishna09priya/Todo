...
router.patch('/projects/:projectId/todos/:todoId/status',authenticate, async (req, res) => {
  const project = await Project.findById(req.params.projectId);
  const todo = project.todos.id(req.params.todoId);
...
const generateGistMarkdown = (project) => {
...
router.get('/projects/:id/export', async (req, res) => {
  const project = await Project.findById(req.params.id);
  const markdownContent = generateGistMarkdown(project);
